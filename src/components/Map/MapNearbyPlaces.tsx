...
import { GoogleMap, MarkerF, InfoWindowF, Circle, DirectionsRenderer } from "@react-google-maps/api";
import Image from "next/image";
import { FaMapMarkerAlt, FaInfoCircle, FaMapSigns, FaExternalLinkAlt } from "react-icons/fa";
...
interface MapNearbyPlacesProps {
...
const MapNearbyPlaces: React.FC<MapNearbyPlacesProps> = ({ center, places, mainPlace, isLoaded }) => {
  const mapRef = useRef<google.maps.Map | null>(null);
  const [selectedEntity, setSelectedEntity] = useState<any>(null);
  const [hoveredMarkerId, setHoveredMarkerId] = useState<number | null>(null); // State for hover effect
  ...
  const calculateRoutes = useCallback(() => {
    ...
    directionsService.route(
      ...
      (result: React.SetStateAction<google.maps.DirectionsResult | null>, status: google.maps.DirectionsStatus) => {
      ...
      >
        ...
        {places.map((entity: any) => (
          <MarkerF
            ...
            animation={hoveredMarkerId === entity.id ? google.maps.Animation.BOUNCE : undefined}
