...
export default function ProfileAccount({ isOpen, onClose }) {
  ...
  const handleVerifyPassword = async (e) => {
    ...
    try {
      ...
      if (response.verified) {
        ...
        MySwal.fire("ยืนยันรหัสผ่านสำเร็จ", "คุณสามารถอัปเดตโปรไฟล์ได้แล้ว", "success");
      } else {
  ...
  const handleUpdateProfile = async (e) => {
    e.preventDefault();
    if (name === "" || username === "") {
      MySwal.fire("เกิดข้อผิดพลาด", "ชื่อและชื่อผู้ใช้ไม่สามารถเว้นว่างได้", "error");
      return;
    }

    if (!passwordVerified) {
      MySwal.fire("เกิดข้อผิดพลาด", "กรุณายืนยันรหัสผ่านก่อนทำการอัปเดตโปรไฟล์", "error");
      return;
    }
  ...
  const handleChangePassword = async (e) => {
    e.preventDefault();
    if (!passwordVerified) {
      MySwal.fire("เกิดข้อผิดพลาด", "กรุณายืนยันรหัสผ่านก่อนเปลี่ยนรหัสผ่าน", "error");
      return;
    }
    ...
    if (newPassword !== confirmNewPassword) {
      MySwal.fire("เกิดข้อผิดพลาด", "รหัสผ่านใหม่และการยืนยันรหัสผ่านไม่ตรงกัน", "error");
      return;
    }
    ...
    try {
      ...
      MySwal.fire("เปลี่ยนรหัสผ่านสำเร็จ", "รหัสผ่านของคุณได้รับการอัปเดตแล้ว", "success");
      resetState();
    } catch (error) {
        ...
        <div className="fixed inset-0 flex items-center justify-center">
          ...
          >
            <Dialog.Panel className="bg-white rounded-lg shadow-lg max-w-full md:max-w-md w-full p-6 mx-4 md:mx-0">
              <Dialog.Title className="text-2xl font-bold mb-6 text-center">โปรไฟล์ผู้ดูแลระบบ</Dialog.Title>
