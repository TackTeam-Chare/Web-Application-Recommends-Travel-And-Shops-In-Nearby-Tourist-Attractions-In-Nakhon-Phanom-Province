      ...
      <div className="max-w-4xl w-full bg-white rounded-lg shadow-md overflow-hidden p-8">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="relative z-0 w-full mb-6 group">
            ...
            {fields.map((item, index) => (
              <div key={item.id} className="grid grid-cols-4 gap-4 mb-4 items-center">
                <select
                  {...register(`operating_hours.${index}.day_of_week`, { required: 'Day of week is required.' })}
                ...
                <input
                  type="time"
                  {...register(`operating_hours.${index}.opening_time`, { required: 'Opening time is required.' })}
                  className="block py-2 px-4 w-full text-sm text-gray-900 bg-transparent border border-gray-300 rounded-md appearance-none focus:outline-none focus:ring-0 focus:border-indigo-600 peer"
                />
                <input
                  type="time"
                  {...register(`operating_hours.${index}.closing_time`, { required: 'Closing time is required.' })}
                  className="block py-2 px-4 w-full text-sm text-gray-900 bg-transparent border border-gray-300 rounded-md appearance-none focus:outline-none focus:ring-0 focus:border-indigo-600 peer"
                />
                <button
                  type="button"
          ...
          <div className="relative z-0 w-full mb-6 group">
                ...
                <div className="mt-4 flex text-sm leading-6 text-gray-600">
                  ...
                  >
                    ...
                    <input
                      ...
                      multiple
                      className="sr-only"
                      {...register('image_paths', { required: 'Please upload images', onChange: handleFileChange })}
                    />
                  </label>
            ...
            {errors.image_paths && <p className="text-red-500 text-xs mt-1">{errors.image_paths.message}</p>}
          </div>
          ...
          <div className="relative z-0 w-full mb-6 group">
            ...
            />
            <label htmlFor="published" className="ml-2 block text-sm leading-5 text-gray-900">Publish</label>
          </div>
